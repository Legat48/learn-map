...
* Что такое `stateful` и `stateless` компоненты?
  ...
  * Что делает хук `useState`? Как обновить состояние?
   позволяет функциональным компонентам иметь внутреннее состояние и управлять им.
**Прямое присвоение** нового значения:

 `setCount(newValue);`

**Функция обратного вызова:**Если новое состояние зависит от предыдущего, рекомендуется использовать функцию обратного вызова. Она гарантирует, что у вас всегда будет актуальная версия состояния.

 `setCount(prevCount => prevCount + 1);`

**Объектное обновление:**Если ваше состояние представляет собой объект, вы можете передать новый объект, содержащий обновленные ключи.
...
function updateName(newName) {
    setPerson({
        ...person,
        name: newName
    }); // Сохраняем старые поля и обновляем имя
}
```

* Как обновить состояние из `useState` на основе предыдущего значения? Какие могут при этом возникнуть проблемы?
  использовать фукцию обратного вызова, так как при прямом присваивании могут произойти ошибки синхронизации изза нескольких обновлений состояния единовременно
 `setCount(prevCount => prevCount + 1);`

* Что такое top-down data flow? Что является основным механизмом синхронизации нескольких компонент?
* Это принцип программирования, когда программа создается пошагово, начиная от общей структуры и до модулей постепенно. Синхронизация компонентов возможна благодаря активации вычислений в узлах их изменений.
