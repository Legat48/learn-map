...
Есть простые типы данных: Число- (±2 в степени 53 -1, 64-битная память) (number), Большое число(символ "n" в конце означает, что это BigInt
)(BigInt) , Строка(string), Строка(), Строка(), Строка(),
Cпециальные числовые значения NaN (не число, вычеслительная ошибка) Infinity (бесконечность)
