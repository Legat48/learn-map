...
HTML-теги - элементы языка, используются для разметки веб страницы
Синематическая верстка нужна для верной работы алгоритмов поисковиков и электронных читалок.
Для синематики нужно использовать синематичные теги, такие как `button`, `h1-6`, `header`, `main`, `footer`,`section`, `nav`, `sarticle`, `aside`, `strong` и тд.

* Что такое `doctype` и зачем он нужен? Что будет, если не написать `doctype`?
Это описание типа документа. Указывает тип документа который используется при написании HTML кода
Существует несколько версий HTML и XHTML(расширенный язык разметки гипертекста). Без указания браузер передет в режим quirks-mode и может неверно отображать страницу.
